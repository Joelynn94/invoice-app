...
export const AppContextProvider: React.FC<{ children: ReactNode }> = ({
  ...
  // Helper function to initialize the invoices state from local storage
  const getInvoicesFromLocalStorage = () => {
    try {
      const invoices = localStorage.getItem("invoices");
      return invoices ? JSON.parse(invoices) : initialMockData;
    } catch (error) {
      console.warn("Error reading invoices from local storage:", error);
      return initialMockData;
    }
  };

  // Initialize the invoices state from local storage
  ...
  // Update local storage only on the client-side whenever invoices change
  useEffect(() => {
